...
class TdxTesting(unittest.TestCase):
    ...
    def test_get_person_by_uid(self):
        ...
        test = self.tdx.get_person_by_uid(standard['ID'])
        self.assertTrue(test)
        self.assertEqual(test['Name'], standard['Name'])
    ...
    def test_edit_tickets(self):
        ...
        if not self.tdx.sandbox:
            return
        ticket_list = list()
        ticket_list.append(self.tdx.get_ticket_by_id(4944908))
        ticket_list.append(self.tdx.get_ticket_by_id(4837287))
        title_string = self.timestamp + ' testing ticket'
        changes = {
            'Title': title_string
        }
        self.assertTrue(all(self.tdx.edit_tickets(ticket_list, changes, notify=False)))
   
    def test_edit_type(self):
        # Protect production from deleting tasks
        if not self.tdx.sandbox:
            return
        ticket = self.tdx.get_ticket_by_id(4944908)
        previous_type = ticket.ticket_data['TypeID']
        desired_type = "IT Internal"
        self.assertTrue(all(self.tdx.edit_ticket_type(ticket, desired_type)))

        # revert change to original type
        self.tdx.edit_ticket_type(ticket, previous_type)
    ...
    def test_edit_status(self):
        ...
        desired_status = "Open"
        self.assertTrue(all(self.tdx.edit_ticket_status(ticket, desired_status)))

        # revert change to original status
        self.tdx.edit_ticket_status(ticket, previous_status)
    ...
    def test_edit_priority(self):
        ...
        self.assertTrue(all(self.tdx.edit_ticket_priority(ticket, desired_priority)))

        # revert change to original priority
        self.tdx.edit_ticket_priority(ticket, previous_priority)
    ...
    def test_edit_urgency(self):
        ...
        desired_urgency = "Low"
        self.assertTrue(all(self.tdx.edit_ticket_urgency(ticket, desired_urgency)))

        # revert change to original urgency
        self.tdx.edit_ticket_urgency(ticket, previous_urgency)
    ...
    def test_edit_impact(self):
        ...
        self.assertTrue(all(self.tdx.edit_ticket_impact(ticket, desired_impact)))

        # revert change to original impact
        self.tdx.edit_ticket_impact(ticket, previous_impact)
    ...
    def test_edit_source(self):
        ...
        self.assertTrue(all(self.tdx.edit_ticket_source(ticket, desired_source)))

        # revert change to original source
        self.tdx.edit_ticket_source(ticket, previous_source)

    # #### GETTING TICKET ATTRIBUTES #### #

    def test_get_all_ticket_types(self):
        types = self.tdx.get_all_ticket_types()
        self.assertGreaterEqual(len(types), 40)

    def test_get_ticket_type_by_id(self):
        key = 23286 # IT Internal
        self.assertTrue(self.tdx.get_ticket_type_by_name_id(key))
    ...
    def test_get_all_ticket_statuses(self):
        statuses = self.tdx.get_all_ticket_statuses()
        self.assertGreaterEqual(len(statuses), 5)

    def test_get_ticket_status_by_id(self):
        key = 30794  # Open
        self.assertTrue(self.tdx.get_ticket_status_by_id(key))

    def test_get_ticket_status_by_name(self):
        key = 'New'
        self.assertTrue(self.tdx.get_ticket_status_by_name(key))

    def test_get_all_ticket_priorities(self):
        priorities = self.tdx.get_all_ticket_priorities()
        self.assertGreaterEqual(len(priorities), 4)

    def test_get_ticket_priority_by_id(self):
        key = 3944  # Low
        self.assertTrue(self.tdx.get_ticket_priority_by_name_id(key))

    def test_get_ticket_priority_by_name(self):
        name = 'Low'
        self.assertTrue(self.tdx.get_ticket_priority_by_name_id(name))

    def test_get_ticket_classification_id_by_name(self):
        name = 'Incident'
        self.assertTrue(self.tdx.get_ticket_classification_id_by_name(name))

    def test_get_all_ticket_urgencies(self):
        urgencies = self.tdx.get_all_ticket_urgencies()
        self.assertGreaterEqual(len(urgencies), 3)

    def test_get_ticket_urgency_by_id(self):
        key = 3941  # Low
        self.assertTrue(self.tdx.get_ticket_urgency_by_name_id(key))
    ...
    def test_get_all_ticket_sources(self):
        sources = self.tdx.get_all_ticket_sources()
        self.assertGreaterEqual(len(sources), 7)

    def test_get_ticket_source_by_id(self):
        key = 1318 # Phone
        self.assertTrue(self.tdx.get_ticket_source_by_name_id(key))
    ...
    def test_create_ticket(self):
        ...
        if not self.tdx.sandbox:
            ...
            json_data = self.tdx.generate_ticket(title_string, "Accounts and Access", "Information Technology",
                                                 "ticketcreation@cedarville.edu")
            created_ticket = self.tdx.create_ticket(json_data)
    ...
    def test_get_all_tasks_by_ticket_id(self):
        ticket_id = 4944908
        self.assertTrue(self.tdx.get_all_tasks_by_ticket_id(ticket_id))

    def test_get_ticket_task_by_id(self):
        ticket_id = 4944908
        task_id = 2290947
        self.assertTrue(self.tdx.get_ticket_task_by_id(ticket_id, task_id))
    ...
    def test_create_ticket_task(self):
        ...
        if not self.tdx.sandbox:
            ...
            created_task = self.tdx.create_ticket_task(ticket_id, task)
    ...
    def test_edit_ticket_task(self):
        ...
        ticket_id = 4944908
        task_id = 2290947
        task = self.tdx.get_ticket_task_by_id(ticket_id, task_id)
        changed_attributes = {
            'Description': f'This task was updated at {dt.now()}.'
        }
        edited_task = self.tdx.edit_ticket_task(4944908, task, changed_attributes)
